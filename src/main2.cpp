...
where_enum where = IN_RED_START; // change change change change change change
...
std::vector<std::string> TargetBall = {"Red_Ball","Black_Ball","Yellow_Ball"}; // change change change change change change
std::string TargetZone = "Red_Placement_Zone"; // change change change change change change

std::string MyBall = "Red_Ball"; // change change change change change change

std::string YourBall = "Blue_Ball";
...
void Submmit_Task()
{
    ...
    while (!isVideoEnd)
    {
        ...
        if(status == StatusPlaceDection)
        {
            ...
            cv::inRange(hsv_region, lower_red1, upper_red1, mask1);
            cv::inRange(hsv_region, lower_red2, upper_red2, mask2);
            ...
            if(isMyBall == true)
            {
                if(color_fill_ratio > 0.85 )
                {
                    IS_IN_DetectZone = true;
                }
            }
            else {
                if(color_fill_ratio > 0.65 )
                {
                    IS_IN_DetectZone = true;
                }
            }
...
void Control_Task()
{
    while (!isVideoEnd)
    {
        ...
        for (auto &obj : Targetobjects)
        {
            ...
            if(isFirsrBall)
            {
                if(obj.className == "Red_Ball" && status == StatusBallDetection)
                {
    ...
    || (center.y < 128))
        ...
        if(status == StatusPlaceDetected && (int)SCRATCH_FLAG == 1)
        {
            ...
            {
                std::cout << "Move forward" << std::endl;
                MOVE_X = (MOVEX_Gain * (zone_side["zone_upside"] - Center.y)) + 0.2;
                status = StatusPlaceDection;
            }
            else if (Center.y > zone_side["zone_downside"])
            {
                std::cout << "Move back" << std::endl;
                MOVE_X = (MOVEX_Gain * (zone_side["zone_downside"] - Center.y)) - 0.4;
                status = StatusPlaceDection;
            }
...
int main()
{
    ...
    // std::thread thread_FuckALL(FuckALL_Function);

    // thread_FuckALL.join();//等待thread_FuckALL线程完成之后再往下执行

    // while(Start_Flag == 0)
    // {
    //     std::cout << "Waiting for Start_Flag = 1" << std::endl;
    // }
