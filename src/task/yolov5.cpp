...
cv::Scalar choose_color(std::string classname)
{
    if (classname == "Black_Ball")
        return cv::Scalar(0, 0, 0); // Black
    else if (classname == "Blue_Ball")
        return cv::Scalar(255, 0, 0); // Blue
    else if (classname == "Blue_Placement_Zone")
        return cv::Scalar(255, 0, 0); // Blue
    else if (classname == "Blue_Start_Zone")
        return cv::Scalar(255, 0, 0); // Blue
    else if (classname == "Cross_Mark")
        return cv::Scalar(0, 255, 0); // Green
    else if (classname == "Red_Ball")
        return cv::Scalar(0, 0, 255); // Red
    else if (classname == "Red_Placement_Zone")
        return cv::Scalar(0, 0, 255); // Red
    else if (classname == "Red_Start_Zone")
        return cv::Scalar(0, 0, 255); // Red
    else if (classname == "Yellow_Ball")
        return cv::Scalar(0, 255, 255); // Yellow
    else
        return cv::Scalar(255, 255, 255); // Default to White
}
...
void DetectionGrp2DetectionArray(yolov5::detect_result_group_t &det_grp, std::vector<Detection> &objects)
{
    for (int i = 0; i < det_grp.count; i++)
    {
        ...
        det.color = choose_color(det.className);
