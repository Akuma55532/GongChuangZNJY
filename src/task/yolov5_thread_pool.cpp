...
// 获取结果，参数：检测框，id（帧号）
nn_error_e Yolov5ThreadPool::getTargetResult(std::vector<Detection> &objects)
{
    // 如果没有结果，等待
    while (results.empty())
    {
    ...
    std::lock_guard<std::mutex> lock(mtx2);
    auto it = results.rbegin(); // 获取最新的结果
    objects = it->second;
    // remove from map
    results.erase(std::next(it).base());

    return NN_SUCCESS;
}

// 获取结果（图片），参数：图片，id（帧号）
nn_error_e Yolov5ThreadPool::getTargetImgResult(cv::Mat &img)
{
    int loop_cnt = 0;
    // 如果没有结果，等待
    ...
    std::lock_guard<std::mutex> lock(mtx2);
    auto it = img_results.rbegin(); // 获取最新的结果
    img = it->second;
    // remove from map
    img_results.erase(std::next(it).base());
