...
public:
    Yolov5ThreadPool();  // 构造函数
    ~Yolov5ThreadPool(); // 析构函数

    nn_error_e setUp(std::string &model_path, int num_threads = 12);     // 初始化
    nn_error_e submitTask(const cv::Mat &img, int id);                   // 提交任务
    nn_error_e getTargetResult(std::vector<Detection> &objects); // 获取结果（检测框）
    nn_error_e getTargetImgResult(cv::Mat &img);                 // 获取结果（图片）
    void stopAll();                                                      // 停止所有线程
};
