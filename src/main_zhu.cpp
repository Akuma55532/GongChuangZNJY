...
std::vector<std::string> TargetBall = {"Blue_Zhu","Black_Zhu","Green_Zhu"}; // change change change change change change
std::string TargetZone = "Blue_Placement_Zone"; // change change change change change change

std::string MyBall = "Blue_Zhu"; // change change change change change change

std::string YourBall = "Red_Zhu";
...
void Control_Task()
{
    while (!isVideoEnd)
    {
        ...
        for (auto &obj : Targetobjects)
        {
            ...
            if(isFirsrBall)
            {
                if(obj.className == "Blue_Zhu" && status == StatusBallDetection)
                {
    ...
    || (center.y < 128))
        ...
        if (status == StatusBallDetected && (int)SCRATCH_FLAG == 0)
        {
            ...
            if (Center.y < ball_side["ball_upside"])
            {
                ...
                MOVE_X = (MOVEX_Gain * (camera_center_y - Center.y)) + 0.2;
                status = StatusBallDetection;
            }
        ...
        if(status == StatusPlaceDetected && (int)SCRATCH_FLAG == 1)
        {
            ...
            {
                std::cout << "Move forward" << std::endl;
                MOVE_X = (MOVEX_Gain * (zone_side["zone_upside"] - Center.y)) + 0.13;
                status = StatusPlaceDection;
            }
            else if (Center.y > zone_side["zone_downside"])
            {
                std::cout << "Move back" << std::endl;
                MOVE_X = (MOVEX_Gain * (zone_side["zone_downside"] - Center.y)) - 0.13;
                status = StatusPlaceDection;
            }
            if (IS_IN_DetectZone && Center.y > zone_side["zone_upside"]-100) {
                // 如果符合条件就抓取,并且将状态设置为blueZone的检测过程中
        ...
        if (status == StatusAfterPlaceDetected && (int)SCRATCH_FLAG == 0)
        {
            ...
            if (Center.y < zone_back_side["zone_back_upside"])
            {
                ...
                MOVE_X = (MOVEX_Gain * (zone_back_side["zone_back_upside"] - Center.y)) + 0.13;
                status = StatusAfterPlaceDetection;
            }
            else if (Center.y > zone_back_side["zone_back_downside"])
            {
                std::cout << "After Move back" << std::endl;
                MOVE_X = (MOVEX_Gain * (zone_back_side["zone_back_downside"] - Center.y)) - 0.13;
                status = StatusAfterPlaceDetection;
            }
        ...
        else if (status == StatusBlueStartDetected && where == NOT_IN_ANY_START && where != IN_RED_START)
        {
            ...
            {
                std::cout << "Move forward" << std::endl;
                MOVE_X = (MOVEX_Gain * (start_side["start_upside"] - Center.y)) + 0.18;
                status = StatusBlueStartDection;
            }
            else if (Center.y > start_side["start_downside"])
            {
                std::cout << "Move back" << std::endl;
                MOVE_X = (MOVEX_Gain * (start_side["start_downside"] - Center.y)) - 0.18;
                status = StatusBlueStartDection;
            }
